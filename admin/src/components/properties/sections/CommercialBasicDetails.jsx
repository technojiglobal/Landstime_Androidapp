...
import { COMMERCIAL_SUB_TYPES, OFFICE_TYPES, RETAIL_TYPES, INDUSTRY_TYPES,HOSPITALITY_TYPES } from '../../../constants/propertyConstants';
...
const CommercialBasicDetails = ({ formData, updateField }) => {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          {COMMERCIAL_SUB_TYPES.map((type) => (
            <button
              ...
              className={`px-4 py-2 rounded-full border text-sm transition-colors ${selectedType === type
          ...
          <div className="flex flex-wrap gap-2">
            {OFFICE_TYPES.map((type) => (
              <button
                ...
                className={`px-4 py-2 rounded-full border text-sm transition-colors ${formData.officeType === type
                    ? 'bg-green-500 text-white border-green-500'
                    : 'bg-white text-gray-700 border-gray-300 hover:border-gray-400'
                  }`}
              >
      ...
      {/* Retail Specific - What kind of retail is it */}
      {/* {selectedType === 'Retail' && (
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {RETAIL_TYPES.map((type) => (
              <button
                ...
                className={`px-4 py-2 rounded-full border text-sm transition-colors ${formData.retailType === type
                    ? 'bg-green-500 text-white border-green-500'
                    : 'bg-white text-gray-700 border-gray-300 hover:border-gray-400'
                  }`}
              >
          ...
          <div className="flex flex-wrap gap-2">
            {INDUSTRY_TYPES.map((type) => (
              <button
                ...
                className={`px-4 py-2 rounded-full border text-sm transition-colors ${formData.industryType === type
                    ? 'bg-green-500 text-white border-green-500'
                    : 'bg-white text-gray-700 border-gray-300 hover:border-gray-400'
                  }`}
              >
      ...
      {/* Hospitality Specific - What kind of hospitality */}
