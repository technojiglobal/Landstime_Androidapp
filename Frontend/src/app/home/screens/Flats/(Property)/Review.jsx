      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View className="border-b border-gray-200 pb-4 mb-4">
            ...
            <Text className="mt-2 text-gray-900 font-bold">{item.title}</Text>
            <Text className="text-gray-700 text-xs leading-5 mt-1">
              {item.comment}
            </Text>

            <View className="flex-row mt-3 space-x-6 mb-3">
              <TouchableOpacity className="flex-row items-center space-x-1 mr-1">
                <Ionicons name="thumbs-up-outline" size={16} color="black" />
                <Text className="black text-[12px] mr-3 ml-1">
                  Helpful ({item.helpful})
                </Text>
              </TouchableOpacity>

              <TouchableOpacity className="flex-row items-center space-x-2 ml-3 mr-3">
                <Ionicons name="thumbs-down-outline" size={16} color="black" />
              </TouchableOpacity>

              <TouchableOpacity className="ml-2 flex-row items-center space-x-1">
                <Ionicons name="chatbox-outline" size={16} color="black" />
                <Text className="black text-sm ml-1">Reply</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
