...
export default function PropertyFormScreen() {
  ...
  return (
      ...
      <TouchableOpacity
        onPress={() => { const currentStorageData = {
              location,
              neighborhoodArea,
              plotArea,
              unit,
  ...
  router.push({
    ...
    >
      ...
      <Text className="text-[14px] font-medium text-[#00000099] mb-3">
        {t('storage_area_neighborhood')}<Text className="text-red-500">*</Text>
      </Text>
      <View
        className="flex-row items-center rounded-md p-3 mb-5"
        style={{
      ...
      >
        ...
        />
        <TextInput
          placeholder={t('storage_enter_area')}
          value={neighborhoodArea}
          onChangeText={setNeighborhoodArea}
          onFocus={() => setFocusedField("neighborhoodArea")}
          onBlur={() => setFocusedField(null)}
          ...
          <View>
            ...
            >
              <Text className="text-base text-gray-700">
                {possessionBy || t('storage_expected_by')}
              </Text>
              <Ionicons name="chevron-down" size={20} color="#666" />
            </TouchableOpacity>
            <Modal visible={visible === "possession"} transparent animationType="slide">
              ...
              >
                <View className="w-[90%] max-h-[50%] bg-white rounded-xl p-2 shadow-md">
                  <FlatList
                    ...
                    renderItem={({ item }) => (
                      <TouchableOpacity
                        className={`p-3 border-b border-gray-200 ${item === possessionOptions[0] ? "bg-[#22C55E]" : ""}`}
                        onPress={() => {
                          // ✅ Store the ENGLISH value, not the translated text
                          const englishValues = [
                            "Immediate",
                            "Within 3 months",
                            "Within 6 months",
                          ...
                          const index = possessionOptions.indexOf(item);
                          setPossessionBy(englishValues[index]);
                          setVisible(null);
                        }}
                      >
                        <Text className={`text-base ${item === possessionOptions[0] ? "text-white font-medium" : "text-gray-700"}`}>
                          {item}
                        </Text>
                      </TouchableOpacity>
                    )}
