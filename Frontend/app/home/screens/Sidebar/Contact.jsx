...
export default function ContactForm() {
  ...
  // Button enable logic
  useEffect(() => {
    const ready = name.trim() && phone && agent && accepted;
    ...
    <SafeAreaView className="flex-1 mt-12 items-center bg-white">
     <StatusBar barStyle="dark-content" backgroundColor="#ffffff" />
        ...
        <View className="flex-row space-x-3">
          {["yes", "no"].map((opt) => (
            <TouchableOpacity
              ...
              className={`w-[84px] h-[34px] rounded-[8px] mx-3 items-center justify-center border border-[#0000001A] ${
                agent === opt ? "bg-[#22C55E1A]" : "bg-white"
              }`}
            >
              <Text className="text-[14px] text-[#000000CC] capitalize">
                {opt}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      <CustomAlertModal
        ...
        message="Your details have been successfully sent to the agent. The agent will contact you soon."
