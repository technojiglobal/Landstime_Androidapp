...
console.log('🔐 Credential verification:', {
  ...
  userNameInDB: user.name
    ...
    // ✅ CHANGED: Check quota ONLY if it's a new property (not already viewed)
const remainingViews = user.currentSubscription.contactViewsRemaining || 0;

// ✅ Allow access if already viewed, even with zero quota
...
export const recordPropertyView = async (req, res) => {
  try {
    ...
    // ✅ NEW: Get or create PropertyView document for this property
...
if (!propertyView) {
  ...
  propertyView = new PropertyView({
    ...
    ownerPhone: property.ownerDetails.phone || 'N/A',  // ✅ Added fallback
    ownerEmail: property.ownerDetails.email || 'N/A',  // ✅ Added fallback
    viewers: [],
    totalViews: 0
  });
} else {
  // Update property status in case it changed
  propertyView.propertyStatus = property.propertyStatus || 'Available';
  propertyView.ownerPhone = property.ownerDetails.phone || 'N/A';  // ✅ Added fallback
  propertyView.ownerEmail = property.ownerDetails.email || 'N/A';  // ✅ Added fallback
}

// ✅ Add viewer to the array
...
export const getPropertyViewers = async (req, res) => {
  try {
    ...
    console.log('📋 Getting viewers for propertyId:', propertyId);
    ...
    // ✅ Find the PropertyView document for this property
    const propertyView = await PropertyView.findOne({ propertyId });
    ...
    if (!propertyView) {
      return res.status(404).json({
        ...
        message: 'No views found for this property'
      });
    }

    // ✅ Paginate viewers array
    const startIndex = (page - 1) * limit;
    const endIndex = page * limit;
    const paginatedViewers = propertyView.viewers
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        propertyTitle: propertyView.propertyTitle,
        propertyOwnerName: propertyView.propertyOwnerName,
        viewers: paginatedViewers,
        totalViews: propertyView.totalViews,
        currentPage: parseInt(page),
        totalPages: Math.ceil(propertyView.viewers.length / limit)
      }
    });
...
export const getAllPropertyViews = async (req, res) => {
  try {
    ...
    // ✅ Filter by subscription plan in viewers array
    if (subscriptionPlan) {
      query['viewers.subscriptionPlan'] = subscriptionPlan;
    }
    ...
    if (startDate || endDate) {
      ...
      query['viewers.viewedAt'] = dateQuery;
    }
...
// ==================== MIGRATION: UPDATE OWNER CONTACTS ====================
// ==================== MIGRATION: UPDATE OWNER CONTACTS ====================
export const migrateOwnerContacts = async (req, res) => {
  try {
    console.log('🔄 Starting migration of owner contacts...');
    ...
    for (const pv of propertyViews) {
      try {
        ...
        if (property && property.ownerDetails) {
          pv.ownerPhone = property.ownerDetails.phone || 'N/A';
          pv.ownerEmail = property.ownerDetails.email || 'N/A';
          await pv.save();
          updated++;
          console.log(`✅ Updated: ${pv.propertyTitle} - Phone: ${pv.ownerPhone}, Email: ${pv.ownerEmail}`);
        } else {
